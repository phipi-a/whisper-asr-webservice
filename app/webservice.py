...
import numpy as np
import ffmpeg
from fastapi import FastAPI, File, UploadFile, Query, applications, Depends
...
@app.post("/asr", dependencies=[Depends(JWTBearer())], tags=["Endpoints"])
def asr(
...
@app.post("/detect-language", dependencies=[Depends(JWTBearer())], tags=["Endpoints"])
def detect_language(
    audio_file: UploadFile = File(...),
    encode: bool = Query(
        default=True, description="Encode audio first through ffmpeg")
